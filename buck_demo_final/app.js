...
function initUI(){
  ...
  $("#reset-btn").addEventListener("click", ()=>{
    Object.assign(params, {Vi:12, fsw:300, D:0.5, L:10, C:100, R:5});
...
function drawPlot(ctx, canvas, series, yMin, yMax, t){
  ...
  for(let i=0;i<=5;i++){ const y=padT+plotH*i/5; ctx.beginPath(); ctx.moveTo(padL,y); ctx.lineTo(padL+plotW,y); ctx.stroke();
    const val = (yMax - (yMin))* (i/5) + yMin; ctx.fillStyle="#475569"; ctx.font="12px system-ui"; ctx.textAlign="right"; ctx.textBaseline="middle"; ctx.fillText(val.toFixed(2), padL-6, y);
  }
...
function updateWaveforms(){
  const twoPeriods = $("#twoPeriods").checked;
  const wf = compute(twoPeriods);
  const t = wf.t;
...
if (iMin>=iMax) iMax=iMin+1;
  ...
  [...chosenV, ...chosenI].forEach(name=>{
    ...
    sw.style.background = VCOLORS[name] || ICOLORS[name] || "#111827";
